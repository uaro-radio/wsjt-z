...
void QSYMessage::getBandModeFreq()
{
  ...
  if(receivedMessage.at(0) =='$' && receivedMessage.size() >= 5)
  {
    ...
    if (bhList.size() >= 3) {
      ...
      ui->label->setText(receivedMessage);
    }
    ui->yesButton->hide();
    ui->noButton->hide();
  }
  else if(receivedMessage.size() >= 5 && receivedMessage.at(0) =='Z' && receivedMessage.at(1) =='A') {
  ...
  else if(receivedMessage.size() >= 5 && (receivedMessage.at(0).isLetter() || receivedMessage.at(0) == '9' || receivedMessage.at(0) == '4' || receivedMessage.at(0) == '7'))
  {
    QString bandParam = "";
